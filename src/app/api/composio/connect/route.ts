...
export async function POST(request: NextRequest) {
  try {
    ...
    // Twitter/X uses OAuth1 — pass the correct scheme
    const twitterSlugs = ['twitter', 'x', 'twitter_v2'];
    const authScheme = twitterSlugs.includes(toolkit.toLowerCase()) ? 'OAUTH1' : 'OAUTH2';
    const authState = await router.initiateAuth(session.id, toolkit, authScheme);
