...
export default function MemoryPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">
            {memories.length > 0 ? `${memories.length} stored ${memories.length === 1 ? 'memory' : 'memories'}` : 'No memories yet — add one to get started.'}
          </p>
        </div>
      ...
      {/* Main Search & Filters */}
      <div className="space-y-3 sticky top-0 z-20 bg-background/95 backdrop-blur-md pb-4 pt-1 border-b border-border/40">
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-4 border border-border/30 rounded-3xl bg-card/50">
              ...
              <div className="text-center px-6">
                <h3 className="font-bold text-lg mb-1">
                  {memorySearchQuery ? 'No matches found' : 'No memories yet'}
                </h3>
                <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                  {memorySearchQuery
                    ? 'Try a different search term or clear the filter.'
                    : 'Click "Add Memory" to store your first piece of knowledge.'}
                </p>
              </div>
              {!memorySearchQuery && (
                <button
