...
import { SaveAgentDialog } from '@/components/agent/save-agent-dialog';
import { MyAgentsDrawer } from '@/components/agent/my-agents-drawer';
...
export default function AgentPage() {
  ...
  // Persistent agents (Save as Agent + NANDA publishing)
  ...
  // Handle /agent?import=<slug|urn> deep link → clone into My Agents.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const importRef = params.get('import');
    if (!importRef) return;

    const body = importRef.startsWith('urn:')
      ? { locator: importRef }
      : { slug: importRef };
    ...
    fetch('/api/agents/import', {
      ...
      .then(async (res) => {
        ...
        if (res.ok) {
          setImportNotice(`Imported "${data.agent.name}" into My Agents.`);
      ...
      .catch(() => setImportNotice('Import failed'))
      .finally(() => {
        // Clean the param so a refresh doesn't re-import.
  ...
  // Map the current run config to the persisted runConfig shape.
  const buildRunConfig = (): Record<string, number> => {
    if (mode === 'timed') return { durationMinutes: duration, intervalMinutes };
    if (mode === 'interval') return { intervalMinutes };
    return {};
  };
        ...
        <div className="px-4 py-3 md:px-6 md:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-semibold tracking-tight">Autonomous Agent</h1>
...
- Check calendar for upcoming events and send reminders"
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  title="Save this task as a reusable, publishable agent"
                >
