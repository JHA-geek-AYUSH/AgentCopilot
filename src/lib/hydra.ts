...
function getBaseParams(userId?: string) {
  return {
    ...
    sub_tenant_id: userId || process.env.NEXT_PUBLIC_HYDRA_DB_SUB_TENANT_ID || 'agentos',
  };
}
        ...
        memories: [
          {
            ...
            title: options?.title || `Memory from agentos`,
...
export async function verifyProcessing(sourceIds: string[], userId?: string): Promise<ProcessingStatus[]> {
  ...
  try {
    ...
    const subTenantId = userId || process.env.NEXT_PUBLIC_HYDRA_DB_SUB_TENANT_ID || 'agentos';
...
export async function storeKnowledge(
  ...
  try {
    const response = await fetch(`${HYDRA_API_URL}/ingestion/upload_knowledge`, {
      ...
      body: JSON.stringify({
        tenant_id: HYDRA_TENANT_ID,
        sub_tenant_id: knowledge.userId || process.env.NEXT_PUBLIC_HYDRA_DB_SUB_TENANT_ID || 'agentos',
        upsert: true,
        app_sources: [
          {
            id: knowledge.id,
