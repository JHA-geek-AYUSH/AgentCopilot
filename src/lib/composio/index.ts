...
export async function executeComposioToolCall(clerkUserId: string, toolCall: { name: string; parameters: any }) {
  // Guard: planners sometimes emit a placeholder step (tool: "none"/"null"/empty) when no
  // real action is needed, instead of just returning a direct answer with zero steps.
  // Forwarding that literal string to Composio always 404s and gets retried pointlessly
  // for every plugin (Gmail, GitHub, Slack, etc.) — so short-circuit it here, once, centrally.
  const normalizedName = (toolCall.name || '').trim().toLowerCase();
  if (!normalizedName || ['none', 'null', 'n/a', 'na', 'noop', 'no_action'].includes(normalizedName)) {
    console.log(`[ComposioIndex] Skipping no-op tool call: "${toolCall.name}"`);
    return { success: true, data: { skipped: true, reason: 'No tool action needed' } };
  }

  try {
    const router = getToolRouter();
    // Using Clerk ID as entityId for Composio session
    const session = await router.getOrCreateSession(process.env.COMPOSIO_PROJECT_NAME || "agentos-main", clerkUserId);
